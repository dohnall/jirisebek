...
(function($) {
	...
	$.extend(ImageSelection.prototype, {
		...
		/* CFMOD this method uses external variables x, y, t (being this), changedW, changedH 
		 * x,y is start of selection, w,h is width, height 
		 * 
		 */
		limitRectInCrop : function(data){	
			...
			if (data.w > this.maxW - data.x){
				data.w=this.maxW - data.x;
				data.h=Math.round(1.0*data.w*this.settings.cf_constrain_for_crop);				
				data.changedW=true;
			}					
			if (data.h > this.maxH - data.y){
				data.h=this.maxH - data.y;
				data.w=Math.round(1.0*data.h*1/this.settings.cf_constrain_for_crop);
				data.changedH=true;
			}
			if (!data.changedW&&!data.changedH){
				if (data.w>10){
					data.h=Math.round(1.0*data.w*this.settings.cf_constrain_for_crop);
				}
			}
			...
			t.startDrag(e, {
				...
				drag : function(cx, cy) {
					...
					if (ty != 'move') {
						...
						if (t.mode=='crop'&&p&&t.settings.cf_constrain_for_crop>0){ /* CFMOD */
							var changedW=false;
							var changedH=false;
							var data={x:x, y:y, w:w, h:h, changedW:changedW, changedH:changedH};
							data=t.limitRectInCrop(data);
							
							/* when there is a change, it's necessary to call it again to decrease size
							 * it's necessary in case when vertical size is changed (then horizontal must be checked)
							 */
							if (data.changedW||data.changedH){
								data=t.limitRectInCrop(data);
							}
		...
		/* CFMOD */
		setSelectionRectExternal:function(){  
			this.cornersVisible = 0;
			this.container.find('span').hide();			
			this.cornersVisible = 1;
			this.drawCorners();
		}
